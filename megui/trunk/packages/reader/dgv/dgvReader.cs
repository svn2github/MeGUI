...
namespace MeGUI
{
    public class dgvFileFactory : IMediaFileFactory
    {
        ...
        {
            return new dgvFile(file);
        }

        public int HandleLevel(string file)
        {
            if (file.ToLower().EndsWith(".dgv"))
                return 12;
            return -1;
        }
    ...
    /// <summary>
    /// Summary description for dgvReader.
    /// </summary>
    public class dgvFile : IMediaFile
    {
        ...
        public dgvFile(string fileName)
        {
            ...
            reader = AvsFile.ParseScript("VC1Source(\"" + this.fileName + "\")");
