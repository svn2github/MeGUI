...
namespace MeGUI
{
    public partial class DGAinputWindow : System.Windows.Forms.Form
    {
        #region variables
        private DGAIndexJob lastJob = null;
        ...
        public DGAinputWindow(MainForm mainForm)
        {
            InitializeComponent();
        ...
        public DGAinputWindow(MainForm mainForm, string fileName, bool autoReturn) : this(mainForm, fileName)
        {
            openVideo(fileName);
        ...
        private void queueButton_Click(object sender, EventArgs e)
        {
            if (Drives.ableToWriteOnThisDrive(Path.GetPathRoot(projectName.Text)))
            {
                if (configured)
                {
                    if (!dialogMode)
                    {
                        DGAIndexJob job = generateIndexJob();
                        lastJob = job;
                    ...
                    MessageBox.Show("You must select a Video Input and DGAVCIndex project file to continue",
        ...
        private DGAIndexJob generateIndexJob()
        {
            ...
            return new DGAIndexJob(this.input.Filename, this.projectName.Text, demuxType, audioTracks, null, loadOnComplete.Checked);
        }
        ...
        /// </summary>
        public DGAIndexJob Job
        {
            get { return generateIndexJob(); }
        }

        public DGAIndexJob LastJob
        {
            get { return lastJob; }
            set { lastJob = value; }
        }
