...
namespace MeGUI.packages.tools.hdbdextractor
{
    public class HdBdStreamExtractor : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            this.LogTextBox.Size = new System.Drawing.Size(558, 2);
            ...
            this.HelpButton2.Location = new System.Drawing.Point(12, 433);
            ...
            this.QueueButton.Location = new System.Drawing.Point(414, 433);
            ...
            this.CancelButton2.Location = new System.Drawing.Point(495, 433);
            ...
            this.Eac3toLinkLabel.Location = new System.Drawing.Point(57, 438);
            ...
            // FeatureButton
            // 
            this.FeatureButton.Location = new System.Drawing.Point(333, 499);
        ...
        public HdBdStreamExtractor(MainForm info)
        {
            ...
            //eac3toPath = @"E:\Disc Backup\eac3to\eac3to.exe";
        ...
        private void FolderInputSourceButton_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                myinput = openFileDialog1.FileName;
            }
...
/*
            ...
            {
                MessageBox.Show("Retrieve features prior to enqueueing job.", "Enqueue Job", MessageBoxButtons.OK, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button1);
                return;
            }
*/
...
//#else
      ...
      /*      HDStreamsExJob job = new HDStreamsExJob(this.FolderInputTextBox.Text, this.FolderOutputTextBox.Text, args.featureNumber, args.args);
            lastJob = job;
            ...
            foreach (DataGridViewRow row in StreamDataGridView.Rows)
            {
                ...
                if (extractStream.Value != null && int.Parse(extractStream.Value.ToString()) == 1)
                {
                    ...
                    sb.Append(string.Format("{0}:\"{1}\" {2} ", stream.Number,
                        ...
                        sb.Append(string.Format("{0}:\"{1}\" {2} ", stream.Number,
                            System.IO.Path.Combine(FolderOutputTextBox.Text, string.Format("{0}_{1}_{2}.{3}", "Track", stream.Number, Extensions.GetStringValue(stream.Type), row.Cells["StreamExtractAsComboBox"].Value).ToLower()),
                            row.Cells["StreamAddOptionsTextBox"].Value).Trim());
