...
new JobProcessorFactory(new ProcessorFactory(init), "x264Encoder");
        ...
        public static string generateX264CLICommandlineMid(x264Settings xs, Dar? d, int hres, int vres)
        {
            StringBuilder sb = new StringBuilder();
            ...
            if (d.HasValue)
            {
                ulong parX = d.Value.X, parY = d.Value.Y;
                if (hres > 0 && vres > 0)
                {
        ...
        public static string genCommandline(string input, string output, Dar? d, int hres, int vres, x264Settings xs)
        {
            ...
            string cmd = generateX264CLICommandlineStart(xs);
            cmd += generateX264CLICommandlineMid(xs, d, hres, vres);
            cmd += generateX264CLICommandlineEnd(xs, input, output);
            return cmd;
