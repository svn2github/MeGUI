...
namespace MeGUI
{
    ...
    /// </summary>
    public class MeGUI : System.Windows.Forms.Form
    {
        ...
        private VideoEncoder vEnc; // the class that does the video encoding
        ...
        private AudioEncoder aEnc; // the class that does the audio encoding
        private Muxer muxer; // the class that does the MP4 muxing
        private DGIndexer indexer; // the class that does the mpeg1/2 file indexing
        private AviSynthProcessor avsProcessor;
        ...
        private void InitializeComponent()
        {
            ...
            this.audioContainerLabel.Location = new System.Drawing.Point(290, 78);
            ...
            // queueAudioButton
            // 
            this.queueAudioButton.Location = new System.Drawing.Point(382, 101);
            ...
            // configAudioButton
            // 
            this.configAudioButton.Location = new System.Drawing.Point(228, 74);
            ...
            // addAnalysisPass
            // 
            this.addAnalysisPass.Location = new System.Drawing.Point(11, 130);
            ...
            // queueVideoButton
            // 
            this.queueVideoButton.Location = new System.Drawing.Point(386, 130);
            ...
            // videoContainerLabel
            // 
            this.videoContainerLabel.Location = new System.Drawing.Point(328, 72);
            this.videoContainerLabel.Name = "videoContainerLabel";
            this.videoContainerLabel.Size = new System.Drawing.Size(56, 23);
            ...
            this.fileType.Location = new System.Drawing.Point(386, 74);
            ...
            // 
            this.videoOutputOpenButton.Location = new System.Drawing.Point(418, 46);
            ...
            // jobProgress
            // 
            this.jobProgress.Location = new System.Drawing.Point(126, 344);
            this.jobProgress.Name = "jobProgress";
            this.jobProgress.Size = new System.Drawing.Size(328, 23);
            this.jobProgress.TabIndex = 8;
            // 
            // progressLabel
            // 
            this.progressLabel.Location = new System.Drawing.Point(49, 344);
            this.progressLabel.Name = "progressLabel";
            this.progressLabel.Size = new System.Drawing.Size(60, 15);
            ...
            // 
            this.mnuAVCLevelValidation.Index = 9;
        ...
        {
            this.videoCodec.Items.AddRange(codecs.ListOfVideoCodecs);
            this.videoCodec.SelectedItem = codecs.X264;
            this.audioCodec.Items.AddRange(codecs.ListOfAudioCodecs);
            this.audioCodec.SelectedItem = codecs.NAAC;
        }
        /// <summary>
        ...
        private void autoEncodeButton_Click(object sender, System.EventArgs e)
        {
            ...
            if (cont)
            {
                ...
                myVideo.VideoType = CurrentVideoOutputType;
        ...
        private void pauseButton_Click(object sender, System.EventArgs e)
        {
            if (!this.paused) // we're encoding
            {
                ...
                if (aEnc != null) // we're currently encoding audio
                    aEnc.pause(out error);
                if (muxer != null) // we're currently muxing
                    muxer.pause(out error);
                if (avsProcessor != null) // we're currently avs processing
                    avsProcessor.pause(out error);
            }
            else
            {
                paused = false;
                ...
                string error;
                if (vEnc != null) // currently encoding video
                    vEnc.resume(out error);
                if (aEnc != null) // we're currently encoding audio
                    aEnc.resume(out error);
                if (muxer != null) // we're currently muxing
                    muxer.resume(out error);
                if (avsProcessor != null) // we're currently avs processing
                    avsProcessor.resume(out error);
            }
            updateProcessingStatus();
        }
        #endregion
        ...
        {
            VideoType[] outputTypes = this.videoEncoderProvider.GetSupportedOutput(this.CurrentVideoCodecSettingsProvider.CodecType);
        ...
        {
            AudioType[] outputTypes = this.audioEncoderProvider.GetSupportedOutput(this.CurrentAudioSettingsProvider.CodecType);
        ...
        public bool startEncoding(Job job)
        {
            ...
            if (job is VideoJob)
            {
                vEnc = new VideoEncoder(this.Settings);
                logBuilder.Append("Job is a video job.");
                if (vEnc.setup((VideoJob)job, out error)) // setup succeeded
                {
                    logBuilder.Append(" encoder commandline:\r\n" + job.Commandline + "\r\n");
                    ...
                    pw = new ProgressWindow(JobTypes.VIDEO);
            ...
            if (job is AudioJob)
            {
                aEnc = new AudioEncoder(this.Settings);
                logBuilder.Append("Job is an audio job. Commandline:\r\n" + job.Commandline + "\r\n");
                if (aEnc.setup(job, out error))
                {
                    ListViewItem item = this.queueListView.Items[getJobPosition(job.Name)];
                    item.SubItems[5].Text = "processing";
                    item.SubItems[6].Text = DateTime.Now.ToLongTimeString();
                    ...
                    aEnc.StatusUpdate += new JobProcessingStatusUpdateCallback(aEnc_StatusUpdate);
                    pw.WindowClosed += new WindowClosedCallback(pw_WindowClosed);
                    pw.Abort += new AbortCallback(pw_Abort);
                    pw.setPriority(job.Priority);
                    pw.PriorityChanged += new PriorityChangedCallback(pw_PriorityChanged);
                ...
                {
                    logBuilder.Append("calling setup failed\r\n");
                    this.isEncoding = false;
                    aEnc = null;
                    retval = false;
                }
            ...
            if (job is MuxJob)
            {
                muxer = new Muxer(Settings);
                logBuilder.Append("Job is a mux job. Commandline:\r\n" + job.Commandline + "\r\n");
                if (muxer.setup(job, out error)) // setup succeeded
                {
                    ListViewItem item = this.queueListView.Items[getJobPosition(job.Name)];
                    item.SubItems[5].Text = "processing";
                    item.SubItems[6].Text = DateTime.Now.ToLongTimeString();
                    ...
                    item.SubItems[8].Text = "";
                    job.Status = JobStatus.PROCESSING;
                    pw = new ProgressWindow(JobTypes.MUX);
                    muxer.StatusUpdate += new JobProcessingStatusUpdateCallback(muxer_StatusUpdate);
                    pw.WindowClosed += new WindowClosedCallback(pw_WindowClosed);
                    pw.Abort += new AbortCallback(pw_Abort);
                    pw.setPriority(job.Priority);
                    pw.PriorityChanged += new PriorityChangedCallback(pw_PriorityChanged);
                    ...
                    if (muxer.start(out error))
                    {
                        this.isEncoding = true;
                        retval = true;
                        logBuilder.Append("successfully set up muxer\r\n");
                    }
                    else
                    {
                        logBuilder.Append("starting muxer failed with error " + error + "\r\n");
                        this.isEncoding = false;
                        retval = false;
                    }
                }
            ...
            if (job is IndexJob)
            {
                indexer = new DGIndexer((IndexJob)job, this.path);
                logBuilder.Append("Job is a dgindex job. dgindex commandline:\r\n " + job.Commandline + "\r\n");
                if (indexer.setup())
                {
                    ListViewItem item = this.queueListView.Items[getJobPosition(job.Name)];
                    item.SubItems[5].Text = "processing";
                    ...
                    item.SubItems[8].Text = "";
                    job.Status = JobStatus.PROCESSING;
                    indexer.StatusUpdate += new IndexStatusUpdateCallback(indexer_StatusUpdate);
                    indexer.start();
                    this.isEncoding = true;
                    retval = true;
                    logBuilder.Append("successfully set up dgindex\r\n");
                }
                else // setup failed, probably a program is missing
                {
                    this.isEncoding = false;
            ...
            {
                avsProcessor = new AviSynthProcessor();
                logBuilder.Append("Job is an AviSynth job. No commandline (run internally)\r\n");
                if (avsProcessor.setup(job, out error))
                {
                    ListViewItem item = this.queueListView.Items[getJobPosition(job.Name)];
                    item.SubItems[5].Text = "processing";
                    item.SubItems[6].Text = DateTime.Now.ToLongTimeString();
                    ...
                    item.SubItems[8].Text = "";
                    job.Status = JobStatus.PROCESSING;
                    avsProcessor.StatusUpdate += new JobProcessingStatusUpdateCallback(avsProcessor_StatusUpdate);

                    pw = new ProgressWindow(JobTypes.AVS);
                    pw.WindowClosed += new WindowClosedCallback(pw_WindowClosed);
                    pw.Abort += new AbortCallback(pw_Abort);
                    pw.setPriority(job.Priority);
                    pw.PriorityChanged += new PriorityChangedCallback(pw_PriorityChanged);
                    ...
                    if (avsProcessor.start(out error))
                    {
                        this.isEncoding = true;
                        retval = true;
                        logBuilder.Append("successfully set up avs processor\r\n");
                    }
                    else
                    {
                        this.isEncoding = false;
                        logBuilder.Append("calling start failed with error message: " + error);
                        avsProcessor = null;
                        retval = false;
                    }
                }
                ...
                {
                    this.isEncoding = false;
                    logBuilder.Append("calling setup failed with error message: " + error);
                    avsProcessor = null;
                    retval = false;
                }
            }
        ...
        {
            //MessageBox.Show("This feature is not yet supported", "Feature not supported", MessageBoxButtons.OK, MessageBoxIcon.Error)
            // Prevents exception being thrown when trying to add a job with no data.
            if (!(videoInput.Text.Equals("")) && !(videoOutput.Text.Equals("")))
            {
            ...
            if (File.Exists(fileName))
            {
                ...
                using (Stream s = File.OpenRead(fileName))
                {
                    ...
                    try
                    {
                        ...
                        pathEnv = MeGUI.GetDirectoryName(settings.DgIndexPath) + ";" + pathEnv;
                    ...
                    {
                        MessageBox.Show("Main profile could not be loaded. Is it a valid profile created by MeGUI?", "Error loading profile", MessageBoxButtons.OK);
                        Console.Write(e.Message);
                    }
                }
            }
        ...
        private void postprocessAudioJob(AudioJob firstAudio, VideoJob firstpass)
        {
            if (firstpass.DesiredSize <= 0) // No desired filesize
            {
                logBuilder.Append("No desired size. Continuing without making any changes.\r\n");
                return;
            }
            logBuilder.Append("this series of jobs starts with an audio job and is followed by regular twopass video jobs\r\n");
            AudioJob secondAudio = null;
            VideoJob secondpass = (VideoJob)jobs[firstpass.Next]; // 2nd pass job
            VideoJob thirdpass = null;
            if (jobs[secondpass.Next] is VideoJob)
                thirdpass = (VideoJob)jobs[secondpass.Next];
            logBuilder.Append("The audio job is named " + firstAudio.Name + " the first pass " + firstpass.Name
                + " and the second pass " + secondpass.Name + "\r\n");
            logBuilder.Append("The second pass job has a desired final output size of " + secondpass.DesiredSize
                + " bytes and video bitrate of " +
                secondpass.Settings.BitrateQuantizer + " kbit/s\r\n");
            if (thirdpass != null)
                logBuilder.Append("Third pass job found: " + thirdpass.Name + "\r\n");
            long audio1Size = 0, audio2Size = 0;
            if (firstAudio.Previous != null)
            {
                Job prev = jobs[firstAudio.Previous];
                if (prev is AudioJob)
                {
                    logBuilder.Append("Found an audio job preceding the audio job that has just been completed: " + prev.Name + "\r\n");
                    secondAudio = (AudioJob)prev;
                    try
                    {
                        FileInfo fi = new FileInfo(prev.Output);
                        audio2Size = fi.Length;
                        logBuilder.Append("The size is of the second audio track is "
                            + fi.Length + " bytes\r\n");
                    }
                    catch (IOException e)
                    {
                        logBuilder.Append("IOException when trying to get size of audio. Does the file "
            ...
            try
            {
                FileInfo fi = new FileInfo(firstAudio.Output);
                audio1Size = fi.Length;
                logBuilder.Append("The size of the first audio track is " + fi.Length + " bytes\r\n");
            }
            catch (IOException e)
            {
                logBuilder.Append("IOException when trying to get size of audio. Does the file "
                    + firstAudio.Output + " exist?\r\nStacktrace: " + e.StackTrace + "\r\n");
            }
            MuxJob mux;
            if (thirdpass == null) // 2 passes only
                mux = (MuxJob)jobs[secondpass.Next]; // mux job
            else // 3 passes
                mux = (MuxJob)jobs[thirdpass.Next];
            int bitrate = 0;
            bool isXviD = false;
            if (firstpass.Settings is xvidSettings)
                isXviD = true;
            int videoSize = 0;
            if (mux.MuxType == MuxerType.MP4BOX)
            {
                bitrate = calc.calculateMP4VideoBitrate(audio1Size + audio2Size, firstpass.DesiredSize, firstpass.NumberOfFrames,
                    mux.Overhead, firstpass.Framerate, isXviD, out videoSize);
            }
            else if (mux.MuxType == MuxerType.MKVMERGE)
            {
                AudioType type1 = null;
                AudioType type2 = null;
                if (firstAudio.Settings is NeroAACSettings)
                    type1 = AudioType.MP4AAC;
                else if (firstAudio.Settings is MP3Settings)
                    type1 = AudioType.VBRMP3;
                if (secondAudio != null)
                {
                    if (secondAudio.Settings is NeroAACSettings)
                        type2 = AudioType.MP4AAC;
                    else if (secondAudio.Settings is MP3Settings)
                        type2 = AudioType.VBRMP3;
                }
                bitrate = calc.calculateMKVVideoBitrate(audio1Size, audio2Size, firstpass.DesiredSize,
            ...
            else if (mux.MuxType == MuxerType.AVIMUXGUI)
            {
                ...
                if (firstAudio.Settings.BitrateMode == BitrateManagementMode.CBR)
                    type = AudioType.CBRMP3;
                bitrate = calc.calculateAVIBitrate2(audio1Size, audio2Size, firstpass.DesiredSize,
                    firstpass.NumberOfFrames, firstpass.Framerate, isXviD, type, null,
                    out videoSize);
            }
            logBuilder.Append("Desired video size after substracting audio size is " + videoSize);
            logBuilder.Append("Setting the desired bitrate of the subsequent video jobs to " + bitrate + " kbit/s\r\n");
            jobUtil.updateVideoBitrate(firstpass, bitrate);
            jobUtil.updateVideoBitrate(secondpass, bitrate);
            if (thirdpass != null)
                jobUtil.updateVideoBitrate(thirdpass, bitrate);
        }
        #endregion
        /// <summary>
        ...
        private void pw_PriorityChanged(ProcessPriority priority)
        {
            ...
            if (vEnc != null)
            {
                if (!vEnc.changePriority(priority, out error))
                    log.AppendText("Error when attempting to change priority: " + error);
            }
            if (aEnc != null)
                aEnc.changePriority(priority, out error);
            if (muxer != null)
            {
                if (!muxer.changePriority(priority, out error))
                    log.AppendText("Error when attempting to change priority: " + error);
            }
            if (avsProcessor != null)
            {
                if (!avsProcessor.changePriority(priority, out error))
                    log.AppendText("Error when attempting to change priority: " + error);
            }
        }
        #endregion
        ...
        private void audioTrack_CheckedChanged(object sender, System.EventArgs e)
        {
            ...
            if (current == lastSelectedAudioTrackNumber)
                return;
        ...
        private void abort()
        {
            ...
            if (this.isEncoding)
            {
                if (vEnc != null)
                {
                    if (!vEnc.stop(out error))
                        log.AppendText("Error when trying to stop processing: " + error);
                }
                if (aEnc != null)
                {
                    if (!aEnc.stop(out error))
                        log.AppendText("Error when trying to stop processing: " + error);
                }
                if (muxer != null)
                {
                    if (!muxer.stop(out error))
                        log.AppendText("Error when trying to stop processing: " + error);
                }
                if (avsProcessor != null)
                {
                    if (!avsProcessor.stop(out error))
                        log.AppendText("Error when trying to stop processing: " + error);
                }
                if (indexer != null)
                    indexer.stop();
            ...
            if (this.paused) // aborting directly causes problems so prevent it
            {
                ...
                if (vEnc != null)
                {
                    if (!vEnc.resume(out error))
                        log.AppendText("Error when trying to resume processing: " + error);
                }
                if (aEnc != null)
                {
                    if (!aEnc.resume(out error))
                        log.AppendText("Error when trying to resume processing: " + error);
                }
                if (muxer != null)
                {
                    if (!muxer.resume(out error))
                        log.AppendText("Error when trying to resume processing: " + error);
                }
                if (avsProcessor != null)
                {
                    if (!avsProcessor.resume(out error))
                        log.AppendText("Error when trying to resume processing: " + error);
                }
                if (indexer != null)
                    indexer.resume();
        ...
        private void mnuAVCLevelValidation_Click(object sender, System.EventArgs e)
        {
            ...
            else
            {
                ...
                x264Settings currentX264Settings = (x264Settings)this.codecs.X264.GetCurrentSettings();
        ...
        private void mnuToolsBitrateCalculator_Click(object sender, System.EventArgs e)
        {
            using (Calculator calc = new Calculator())
            {
                ...
                int audio1Bitrate = 0;
                int audio2Bitrate = 0;
                if (this.audioStreams[0].settings != null)
                    audio1Bitrate = audioStreams[0].settings.Bitrate;
                if (this.audioStreams[1].settings != null)
                    audio2Bitrate = audioStreams[1].settings.Bitrate;
                calc.setDefaults(nbFrames, framerate, videoCodec.SelectedIndex, fileType.SelectedIndex,
                    audio1Bitrate, audio2Bitrate);

                DialogResult dr = calc.ShowDialog();
                if (dr == DialogResult.OK)
                {
                    videoCodec.SelectedIndex = calc.getSelectedCodec();
                    VideoCodecSettings settings = CurrentVideoCodecSettingsProvider.GetCurrentSettings();
                    ...
                    fileType.SelectedIndex = calc.getSelectedContainer();
                }
            }
        }
