...
namespace MeGUI
{
    ...
    public class MuxableType
    {
        ...
        public object codec;
        public MuxableType(OutputType type, object codec)
        {
...
//            registeredMuxers.Add(new AVIMuxGUIMuxerProvider() as IMuxing);
        ...
        private MuxPath findBestMuxPathAndConfig(List<object> undecidedInputs, List<MuxableType> decidedInputs, ContainerType containerType)
        {
            ...
            else
            {
                ...
                if (undecidedInput is VideoEncoderType)
                {
                    ...
                    {
                        MuxableType input = new MuxableType(v, CodecManager.VideoCodecFromEncoderType((VideoEncoderType)undecidedInput));
                ...
                if (undecidedInput is AudioEncoderType)
                {
                    ...
                    {
                        MuxableType input = new MuxableType(a, CodecManager.AudioCodecFromEncoderType((AudioEncoderType)undecidedInput));
                        decidedInputs.Add(input);
                        MuxPath path = findBestMuxPathAndConfig(undecidedInputs, decidedInputs, containerType);
                        if (path != null)
                        {
...
/*        private OutputType videoTypeFromContainerType(ContainerType containerType)
        ...
        public List<ContainerFileType> GetSupportedContainers(params MuxableType[] inputTypes)
        {
            List<ContainerFileType> supportedContainers = new List<ContainerFileType>();
            foreach (ContainerFileType cot in GetSupportedContainers())
            {
                if (CanBeMuxed(cot.ContainerType, inputTypes))
                {
        ...
        public List<ContainerFileType> GetSupportedContainers(VideoEncoderType? videoCodec, AudioEncoderType[] audioCodecs,
            params MuxableType[] dictatedOutputTypes)
        {
            ...
            {
                if (CanBeMuxed(videoCodec, audioCodecs, cot.ContainerType, dictatedOutputTypes))
                {
                    if (!supportedContainers.Contains(cot))
                        supportedContainers.Add(cot);
                }
