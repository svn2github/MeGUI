...
namespace MeGUI
{
	...
	public class CommandLineGenerator
	{
		...
		private static string generateX264CLICommandline(x264Settings xs, string input, string output, int parX, int parY)
		{
			...
			if (xs.NbThreads != 1)
				sb.Append("--threads auto "); // forcing x264 thread autodetection
		...
		{
			StringBuilder sb = new StringBuilder();
			string projName = MainForm.GetDirectoryName(output) + @"\" + Path.GetFileNameWithoutExtension(output);
		...
		#endregion
        public static string generateVobSubCommandline(string parameterFile)
        {
            return "vobsub.dll,Configure " + parameterFile;
        }
