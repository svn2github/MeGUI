...
namespace MeGUI
{
	...
	public class VideoPlayer : System.Windows.Forms.Form
	{
		...
		private bool isRunning, isAVS; // whether the player is running and whether the input is an avisynth script
		...
		public bool loadVideo(string path, PREVIEWTYPE type, bool hasAR, bool inlineAvs)
		{
            ...
            if (!inlineAvs)
            {
                if (Path.GetExtension(path).ToLower().Equals(".avs"))
                    isAVS = true;
            }
            else
                isAVS = true;
			...
			{
                if(isAVS)
                    reader = inlineAvs?AvsReader.ParseScript(path):AvsReader.OpenScriptFile(path);  //(VideoReader)new AVIReader(path);
                else
                    reader = (VideoReader)new d2vReader(path, true);
			}
			...
			if (reader != null && reader.FrameCount > 0)
			{
				...
				isRunning = false;
				millisecondsPerFrame = (int)(1000 / reader.Framerate);
				return true;
			}
			return false;
		}
