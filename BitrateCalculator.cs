...
namespace MeGUI
{
	...
	public class BitrateCalculator
	{
        ...
        private int calculateMP4VideoBitrate(AudioStream[] audioStreams, long desiredOutputSize, int nbOfFrames,
            bool useBframes, double framerate, out long videoSize)
        {
        ...
        private int calculateMP4Size(AudioStream[] audioStreams, int desiredBitrate, int nbOfFrames, bool useBframes, double framerate, out int rawVideoSize)
        {
            double mp4Overhead = this.getMP4Overhead(useBframes);
            double totalOverhead = (double)nbOfFrames * mp4Overhead;
            double nbOfSeconds = (double)nbOfFrames / framerate;
            ...
            return (int)(size / (long)1024);
        }
        /// <summary>
		/// calculates the size of the final mp4 file given the audio tracks and desired video bitrate
		/// </summary>
		/// <param name="audioSize">size of the audio</param>
		/// <param name="desiredBitrate">desired video bitrate</param>
		/// <param name="nbOfFrames">number of frames of the video source</param>
		/// <param name="mp4Overhead">overhead of the mp4 video</param>
		/// <param name="framerate">framerate of the video</param>
		/// <param name="isXviD">is the xvid codec used</param>
		/// <param name="rawVideoSize">size of the raw video stream at the given bitrate in KB</param>
		/// <returns>the size of the final mp4 file in KB</returns>
		public int calculateMP4Size(long audioSize, int desiredBitrate, int nbOfFrames, double mp4Overhead, double framerate, bool isXviD, out int rawVideoSize)
		{
			double totalOverhead = (double)nbOfFrames * mp4Overhead;
			double nbOfSeconds = (double)nbOfFrames / framerate;
			if (isXviD) // subtract the 24 bytes/frame AVI overhead that XviD assumes
		...
		#region matroska
		/// <summary>
		/// calculates the video bitrate for the matroska container
		/// </summary>
		/// <param name="audioSize1">size of the first audio track</param>
		/// <param name="audioSize2">size of the second audio track</param>
		/// <param name="desiredOutputSize">desired size of the output</param>
		/// <param name="nbOfFrames">number of frames of the video</param>
		/// <param name="nbBframes">number of b-frames of the video</param>
		/// <param name="framerate">framerate of the video</param>
		/// <param name="isXviD">whether the video is using the xvid codec or not</param>
		/// <param name="audio1Type">type of the first audio track in bytes</param>
		/// <param name="audio2Type">type of the second audio track in bytes</param>
		/// <param name="videoSize">size of the raw video track in KB</param>
		/// <returns>the calculated bitrate in kbit/s</returns>
		public int calculateMKVVideoBitrate(long audioSize1, long audioSize2, long desiredOutputSize, 
			int nbOfFrames, int nbOfBframes, double framerate, bool isXviD, AudioType audio1Type, 
			AudioType audio2Type, out int videoSize)
		{
			...
			double audioOverhead = getMKVAudioOverhead(audio1Type, 48000, nbOfSeconds) +
				getMKVAudioOverhead(audio2Type, 48000, nbOfSeconds);
			if (isXviD)
				totalOverhead -= (double)24 * (double)nbOfFrames;
			long videoTargetSize = desiredOutputSize - audioSize1 - audioSize2 - (long)audioOverhead - 
				(long)totalOverhead;
			videoSize = (int)(videoTargetSize / (long)1024);
			long sizeInBits = videoTargetSize * 8;
			int bitrate = (int)(sizeInBits / (nbOfSeconds * 1000));
        ...
        private int calculateMKVSize(AudioStream[] audioStreams, int desiredBitrate, int nbOfFrames, double framerate, bool useBframes, out int rawVideoSize)
        {
            double totalOverhead = 0.0;
            int nbIframes = nbOfFrames / 10;
            int nbBframes = 0;
        ...
        /// <summary>
		/// calculates the size of a muxed mkv file given video bitrate, and audio stream size
		/// </summary>
		/// <param name="audio1Size">size of the first audio track in bytes</param>
		/// <param name="audio2Size">size of the second audio track in bytes</param>
		/// <param name="desiredBitrate">desired video bitrate</param>
		/// <param name="nbOfFrames">number of frames of the video</param>
		/// <param name="framerate">framerate of the video</param>
		/// <param name="isXviD">is the video codec XviD?</param>
		/// <param name="audio1Type">type of the first audio track</param>
		/// <param name="audio2Type">type ozf the second audio track</param>
		/// <param name="rawVideoSize">size of the raw video stream in KB</param>
		/// <returns>the size of the final product in KB</returns>
		public int calculateMKVSize(long audio1Size, long audio2Size, int desiredBitrate, int nbOfFrames, double framerate, 
			int nbOfBframes, bool isXviD, AudioType audio1Type, AudioType audio2Type, out int rawVideoSize)
		{
			double totalOverhead = 0.0;
			int nbIframes = nbOfFrames / 10;
			int nbBframes = 0;
			...
			long videoSize = (long)(nbOfSeconds * bytesPerSecond);
			rawVideoSize = (int)(videoSize / (long)1024);
			long size = videoSize + (long)audio1Size + (long)audio2Size + (long)totalOverhead;
			return (int)(size / (long)1024);
		}
		/// <summary>
		...
		#region avi
		/// <summary>
		/// calculates the bitrate for a video in an AVI given the audio and video properties
		/// </summary>
		/// <param name="audio1Size">size of the first audio stream (in bytes)</param>
		/// <param name="audio2Size">size of the second audio stream</param>
		/// <param name="desiredOutputSize">desired size of the output (in bytes)</param>
		/// <param name="nbOfFrames">number of frames in the video</param>
		/// <param name="framerate">framerate of the video</param>
		/// <param name="isXviD">is the codec xvid</param>
		/// <param name="audio1Type">type of the first audio stream</param>
		/// <param name="audio2Type">type of the second audio stream</param>
		/// <param name="videoSize">the size of the raw video stream in KB</param>
		/// <returns>the bitrate used to get a video at the desired size in kbit/s</returns>
		public int calculateAVIBitrate2(long audio1Size, long audio2Size, long desiredOutputSize, int nbOfFrames, 
			double framerate, bool isXviD, AudioType audio1Type, AudioType audio2Type, out int videoSize)
		{
			double videoOverhead = (double)nbOfFrames * aviVideoOverhead;
			double nbOfSeconds = (double)nbOfFrames / framerate;
			long audioSize = audio1Size + audio2Size;
			double audio1Overhead = 0;
			double audio2Overhead = 0;
			if (audio1Size > 0)
			{
				audio1Overhead = getAviAudioOverhead(audio1Type) * (double)nbOfFrames;
			}
			if (audio2Size > 0)
			{
				audio2Overhead = getAviAudioOverhead(audio2Type) * (double)nbOfFrames;
			}
			if (isXviD) // xvid already takes the frame overhead into account
				videoOverhead = 0;
			double totalOverhead = videoOverhead + audio1Overhead + audio2Overhead;
			long videoTargetSize = desiredOutputSize - audioSize - (long)totalOverhead;
			videoSize = (int)(videoTargetSize / (long)1024);
			long sizeInBits = videoTargetSize * 8;
			int bitrate = (int)(sizeInBits / (nbOfSeconds * 1000));
		...
		/// <summary>
		/// calculates the size of the output given the size of the audio streams and the desired
		/// video bitrate
		/// </summary>
		/// <param name="audio1Size">size of the first audio track in bytes</param>
		/// <param name="audio2Size">size of the second audio track in bytes</param>
		/// <param name="desiredBitrate">desired video bitrate in kbit/s</param>
		/// <param name="nbOfFrames">number of frames of the video source</param>
		/// <param name="isXviD">whether the codec is xvid</param>
		/// <param name="audio1Type">type of the first audio track</param>
		/// <param name="audio2Type">type of the second audio track</param>
		/// <returns>size of the AVI in KB</returns>
		public int calculateAVISize2(long audio1Size, long audio2Size, int desiredBitrate, int nbOfFrames, double framerate, 
			bool isXviD, AudioType audio1Type, AudioType audio2Type, out int rawVideoSize)
		{
        ...
        private int calculateAVISize(AudioStream[] audioStreams, int desiredBitrate, int nbOfFrames, double framerate, out int rawVideoSize)
        {
            double videoOverhead = this.aviVideoOverhead;
            double nbOfSeconds = (double)nbOfFrames / framerate;
            long audioSize = 0L;
        ...
        public int CalculateBitrate(VideoCodec codec, bool useBframes, ContainerType container, AudioStream[] audioStreams, long desiredOutputSize, int nbOfFrames, double framerate, out long videoSize)
        {
            switch (container)
            {
                case ContainerType.MP4:
                    return calculateMP4VideoBitrate(audioStreams, desiredOutputSize, nbOfFrames, useBframes, framerate, out videoSize);
                case ContainerType.AVI:
                    return calculateAVIBitrate(audioStreams, desiredOutputSize, nbOfFrames, framerate, out videoSize);
                case ContainerType.MKV:
                    return calculateMKVVideoBitrate(audioStreams, desiredOutputSize, nbOfFrames, framerate, useBframes, out videoSize);
            }
            videoSize = 0;
            return 0;
        }
        public int CalculateFileSize(VideoCodec codec, bool useBframes, ContainerType container, AudioStream[] audioStreams, int desiredBitrate, int nbOfFrames, double framerate, out int rawVideoSize)
        {
            switch (container)
            {
                case ContainerType.MP4:
