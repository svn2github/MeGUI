...
namespace MeGUI
{
	...
	public class CommandLineGenerator
	{
        ...
        private static string generateHfyuCommandLine(hfyuSettings ls, string input, string output, int parX, int parY)
        {
            ...
            sb.Append("-of avi -forceidx -noodml "); // Make sure we don't get problems with filesizes > 2GB
		...
		private static string generateX264CLICommandline(x264Settings xs, string input, string output, int parX, int parY)
		{
			...
			if (xs.NbThreads != 1)
				sb.Append("--threads " + xs.NbThreads + " --thread-input ");
			if (xs.Zones != null && xs.Zones.Length > 0 && xs.CreditsQuantizer >= new decimal(1))
			{
				sb.Append("--zones ");
				foreach (Zone zone in xs.Zones)
		...
		#region audio
		/// <summary>
		/// generates a besweet commandline for mp3 audio encoding
		/// </summary>
		/// <param name="ms">audio settings</param>
		...
		private static string generateLameCommandLine(MP3Settings ms, string input, string output)
		{
			...
			sb.Append("-core( -input \"");
			sb.Append(input +  "\" -output \"" + output);
			if (Path.GetExtension(input).ToLower().Equals(".ac3")) // ac3 input
			{
				sb.Append("\" -logfile \"" + besweetLogFile + "\" ) -azid( ");
                if (ms.SourceIsMultichannel && ms.DownmixMode!=ChannelMode.KeepOriginal) // stereo output, trigger 2ch downmix for azid
                {
                    sb.Append("-s ");
                    if (ms.DownmixMode == ChannelMode.StereoDownmix)
                        sb.Append("stereo ");
                    if (ms.DownmixMode == ChannelMode.DPLDownmix)
                        sb.Append("dpl ");
                    if (ms.DownmixMode == ChannelMode.DPLIIDownmix)
                        sb.Append("dplii ");
                    sb.Append("-c normal -L -3db ) -lame( ");
                }
			}
			else // non AC3 input, no azid settings required
			{
				sb.Append("\" -logfile \"" + besweetLogFile + "\" ) -lame(  ");
			}
			if (ms.BitrateMode == BitrateManagementMode.CBR) // cbr output
			{
				sb.Append("-b " + ms.Bitrate + " )");
			}
			else if (ms.BitrateMode == BitrateManagementMode.ABR)
			{
				sb.Append("--abr " + ms.Bitrate + " )");
			}
			else
			{
				sb.Append("-m j -v --vbr-new -V ");
				int qual = Math.Abs((ms.Quality - 10) / 10 - 9); // convert range of 10 - 100 to 9 - 0
				sb.Append(qual + " )");
			}
			if (ms.DelayEnabled || ms.AutoGain)
				sb.Append(" -ota( ");
			if (ms.DelayEnabled)
				sb.Append("-d " + ms.Delay + " ");
			if (ms.AutoGain)
				sb.Append("-g max ");
			if (ms.DelayEnabled || ms.AutoGain)
				sb.Append(")");
		...
		private static string generaceFAACCommandLine(FaacSettings fas, string input, string output)
		{
			...
			{
				sb.Append("\" -logfile \"" + besweetLogFile + "\" ) -azid( ");
                if (fas.SourceIsMultichannel && fas.DownmixMode != ChannelMode.KeepOriginal) // stereo output, trigger 2ch downmix for azid
                {
                    sb.Append("-s ");
                    if (fas.DownmixMode == ChannelMode.StereoDownmix)
                        sb.Append("stereo ");
                    if (fas.DownmixMode == ChannelMode.DPLDownmix)
                        sb.Append("dpl ");
                    if (fas.DownmixMode == ChannelMode.DPLIIDownmix)
                        sb.Append("dplii ");
                    sb.Append("-c normal -L -3db ) -dimzon( -dllname bse_FAAC.dll ");
                }
                if (fas.SourceIsMultichannel && fas.DownmixMode == ChannelMode.KeepOriginal)
                    sb.Append("-c normal ) -dimzon( -dllname bse_FAAC.dll -6chnew ");
			}
			else // non AC3 input, no azid settings required
			{
				sb.Append("\" -logfile \"" + besweetLogFile + "\" ) -dimzon( -dllname bse_FAAC.dll ");
			}
            if (fas.BitrateMode != BitrateManagementMode.VBR) // cbr output
            {
                sb.Append("-b " + fas.Bitrate + " ");
            }
            else
            {
                // vbr output
                sb.Append("-q " + fas.Quality + " ");
            }
			sb.Append(") ");
			if (fas.DelayEnabled || fas.AutoGain)
				sb.Append(" -ota( ");
			if (fas.DelayEnabled)
				sb.Append("-d " + fas.Delay + " ");
			if (fas.AutoGain)
				sb.Append("-g max ");
			if (fas.DelayEnabled || fas.AutoGain)
				sb.Append(")");
		...
		/// <summary>
		/// generates a besweet commandline based on the codec parameters specified as input
		/// </summary>
		...
		private static string generateNeroAACCommandLine(NeroAACSettings nas, string input, string output)
		{
			...
			{
				sb.Append("\" -logfile \"" + besweetLogFile + "\" ) -azid( ");
                if (nas.SourceIsMultichannel && nas.DownmixMode != ChannelMode.KeepOriginal) // stereo output, trigger 2ch downmix for azid
                {
                    sb.Append("-s ");
                    if (nas.DownmixMode == ChannelMode.StereoDownmix)
                        sb.Append("stereo ");
                    if (nas.DownmixMode == ChannelMode.DPLDownmix)
                        sb.Append("dpl ");
                    if (nas.DownmixMode == ChannelMode.DPLIIDownmix)
                        sb.Append("dplii ");
                    sb.Append("-c normal -L -3db ) -bsn( -2ch ");
                }
                if (nas.SourceIsMultichannel && nas.DownmixMode == ChannelMode.KeepOriginal)
                    sb.Append("-c normal ) -bsn( -6chnew ");
			}
			else // non AC3 input, no azid settings required
			{
				sb.Append("\" -logfile \"" + besweetLogFile + "\" ) -bsn( -2ch ");
			}
			if (nas.BitrateMode == BitrateManagementMode.CBR) // cbr output
				if (nas.Bitrate <= 48)
                    sb.Append("-cbr " + nas.Bitrate + " -usepns ");
                else
                    sb.Append("-cbr " + nas.Bitrate + " ");
			if (nas.BitrateMode == BitrateManagementMode.VBR) // vbr output
			{
				sb.Append("-vbr_");
				if (nas.VbrPreset == VBRPPRESET.TAPE)
					sb.Append("tape -usepns ");
				if (nas.VbrPreset == VBRPPRESET.RADIO)
					sb.Append("radio -usepns ");
				if (nas.VbrPreset == VBRPPRESET.INTERNET)
					sb.Append("internet ");
				if (nas.VbrPreset == VBRPPRESET.STREAMING)
					sb.Append("streaming ");
				if (nas.VbrPreset == VBRPPRESET.NORMAL)
					sb.Append("normal ");
				if (nas.VbrPreset == VBRPPRESET.EXTREME)
					sb.Append("extreme ");
				if (nas.VbrPreset == VBRPPRESET.AUDIOPHILE)
					sb.Append("audiophile ");
				if (nas.VbrPreset == VBRPPRESET.TRANSCODING)
					sb.Append("transcoding ");
			}
            if (nas.QualityEnabled)
			{
				if (nas.Quality == QUALITYMODE.QUALITY_FAST)
					sb.Append("-codecquality_fast ");
				if (nas.Quality == QUALITYMODE.QUALITY_HIGH)
					sb.Append("-codecquality_high ");
			}
			if (nas.ProfileEnabled)
			{
				if (nas.Profile == AACLEVEL.PROFILE_HE)
					sb.Append("-aacprofile_he ");
				if (nas.Profile == AACLEVEL.PROFILE_LC)
					sb.Append("-aacprofile_lc ");
			}
			sb.Append(")");
			if (nas.DelayEnabled || nas.AutoGain)
				sb.Append(" -ota( ");
			if (nas.DelayEnabled)
				sb.Append("-d " + nas.Delay + " ");
			if (nas.AutoGain)
				sb.Append("-g max ");
			if (nas.DelayEnabled || nas.AutoGain)
				sb.Append(")");
		...
		{
			StringBuilder sb = new StringBuilder();
			string projName = MeGUI.GetDirectoryName(output) + @"\" + Path.GetFileNameWithoutExtension(output);
			sb.Append("\"" + DGIndexPath + "\" ");
			sb.Append("-AIF=[" + input + "] -OF=[" + projName + "] -exit ");
			if (demuxType == 2)
