...
namespace MeGUI
{
	...
	public class JobUtil
    {
        ...
        public MuxJob[] GenerateMuxJobs(VideoStream video, SubStream[] audioStreamsArray, AudioType[] audioTypes,
            SubStream[] subtitleStreamsArray, SubtitleType[] subTypes,
            string chapterFile, ContainerType container, string output, int splitSize)
        {
            MuxProvider prov = new MuxProvider();
            MuxPath muxPath = prov.GetMuxPath(video.VideoType, audioTypes, subTypes, container, 1 + audioStreamsArray.Length);
            ...
            foreach (MuxPathLeg mpl in muxPath)
            {
                ...
                tempNumber++;
                foreach (OutputType o in mpl.handledInputTypes)
                {
                    if (o is VideoType)
                    {
                    ...
                    else if (o is AudioType)
                    {
                        ...
                        {
                            if (VideoUtil.guessAudioType(audioStream.path).Equals(o))
                            {
                    ...
                    else if (o is SubtitleType)
                    {
                        foreach (SubStream subStream in subtitleStreams)
                        {
                            if ((VideoUtil.guessSubtitleType(subStream.path).Equals(o)))
                            {
...
/*		/// <summary>
		...
		public VideoJob[] prepareVideoJob(string movieInput, string movieOutput, VideoCodecSettings settings, bool prerender, bool checkVideo)
		{
            ...
            if (prerender)
            {
                hfyuFile = MeGUI.GetDirectoryName(movieInput) + "\\hfyu_" + 
			...
			if (job != null)
			{
				if (twoPasses || threePasses) // we just created the last pass, now create previous one(s)
				{
					...
					if (threePasses)
					{
                        ...
                        {
                            middlepass.Output = MeGUI.GetDirectoryName(job.Output) + @"\" + Path.GetFileNameWithoutExtension(job.Output)
		...
		public void postprocessIndexJob(IndexJob job)
		{
			...
			else if (job.PostprocessingProperties != null) // full postprocessing mode
			{
				...
				string videoOutput = Path.Combine(MeGUI.GetDirectoryName(job.Output),
                ...
                myVideo.ParX = parX;
                myVideo.ParY = parY;
                myVideo.VideoType = (new VideoEncoderProvider().GetSupportedOutput(videoSettings.Codec))[0];
...
/*                    vUtil.generateJobSeries(videoInput, videoOutput, muxedOutput, videoSettings,
		...
		{
			sarX = sarY = -1;
			VideoReader reader = (VideoReader)new d2vReader(path);
			...
			if (aspectRatio == (int)AspectRatio.A16x9)
				customDAR = 1.778;
			else if (aspectRatio == (int)AspectRatio.A4x3)
				customDAR = 1.333;
			else if (aspectRatio == (int)AspectRatio.A1x1)
				customDAR = 1;
			...
			if (aspectRatio == 4) // it does
			{
				...
				switch (((d2vReader)reader).AR)
				{
					case (int)AspectRatio.A16x9:
						customDAR = 1.778;
						logBuilder.Append("Found aspect ratio of 16:9.\r\n");
						break;
					case (int)AspectRatio.A4x3:
						customDAR = 1.333;
						logBuilder.Append("Found aspect ratio of 4:3.\r\n");
						break;
					case (int)AspectRatio.A1x1:
						customDAR = 1;
						logBuilder.Append("Found aspect ratio of 1:1.\r\n");
						break;
					default:
						customDAR = 1.778;
						logBuilder.Append("No aspect ratio found, defaulting to 16:9\r\n");
						break;
				}
			}
